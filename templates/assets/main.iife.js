...
Navigation:
  ...
  Tab           - Auto-complete
  Enter         - Execute
`.trim();function ca(_,$){if(!_||_===".")return{};if(_===".."||_==="../"){const i=oa($),T=ve(i);return T?{navigate:T}:{newPath:i}}const e=De(_,$),t=We(e);if(!t){const i=ve(e);return i?{navigate:i}:{output:`cd: no such file or directory: ${_}`}}const n=t.find(i=>i.type==="file"&&i.name===_.split("/").pop());if(n?.permalink)return{navigate:n.permalink};const r=ve(e);return r?{navigate:r}:{newPath:e}}function zn(_,$){const e=_?De(_,$):$,t=We(e);if(!t)return`ls: cannot access '${e}': No such file or directory`;if(t.length===0)return"(empty directory)";const n=t.map(r=>{const i=r.type==="dir"?"drwxr-xr-x":"-rw-r--r--",T=r.type==="dir"?"-":"4.0K",o=r.date??"Mar 30 12:00",E=r.type==="dir"?"/":"";return`${i}  ${T} ${aa()}  staff  ${o}  ${r.name}${E}`});return`Total ${t.length}
${n.join(`
`)}`}function Qn(_){const $=_==="next"?window.haloData?.nextPost:window.haloData?.prevPost;return $?(C_($),{}):{output:_==="next"?"No next article available.":"No previous article available."}}function s$(_){const $=window.haloData?.pagination;if(!$)return{output:"Pagination not available on this page."};const e=_?$.nextUrl:$.prevUrl;return e?(C_(e),{}):{output:_?"Already at the last page.":"Already at the first page."}}function fa(_){if(!_){const $=window.SearchWidget;return typeof $<"u"&&$&&typeof $.open=="function"?$.open():C_("/search"),{}}return C_(`/search?keyword=${encodeURIComponent(_)}`),{}}const Ra=new Map([["back",()=>(window.history.back(),{})],["cd",(_,$)=>ca(_,$)],["clear",()=>({})],["help",()=>({showHelp:!0})],["ll",(_,$)=>({output:zn(_,$)})],["ls",(_,$)=>({output:zn(_,$)})],["next",()=>Qn("next")],["npage",()=>s$(!0)],["pd",()=>s$(!0)],["ppage",()=>s$(!1)],["prev",()=>Qn("prev")],["pu",()=>s$(!1)],["search",_=>fa(_)]]);async function da(_,$,e){const t=Ra.get(_.toLowerCase());return t?t($,e):{output:`bash: ${_}: command not found. Type 'help' for available commands.`}}function ha(){$_.data("terminalInput",(_="~/blog")=>({command:"",completionState:{candidates:[],index:0},currentPath:_,history:[],historyIndex:-1,output:"",showHelp:!1,autoComplete(){const $=this.command,e=this.completionState.candidates[this.completionState.index];this.completionState.candidates.length>0&&$===e?this.completionState.index=(this.completionState.index+1)%this.completionState.candidates.length:(this.completionState.candidates=Ia($,String(this.currentPath),this.isPostPage()),this.completionState.index=0),this.completionState.candidates.length>0&&(this.command=this.completionState.candidates[this.completionState.index])},async executeCommand(){const $=this.command.trim();if(!$)return;this.history[this.history.length-1]!==$&&this.history.push($),this.historyIndex=this.history.length;const[e,...t]=$.split(/\s+/),n=t.join(" ");this.showHelp=!1,this.output="",(e.toLowerCase()==="ls"||e.toLowerCase()==="ll")&&(this.output="Loading...",await new Promise(i=>window.setTimeout(i,50)));const r=await da(e,n,String(this.currentPath));this.applyResult(r),this.command=""},applyResult($){$.navigate?C_($.navigate):$.newPath?(this.currentPath=$.newPath,this.output=""):$.showHelp?this.showHelp=!0:$.output&&(this.output=$.output)},get helpText(){return this.isPostPage()?ua:la},handleKeydown($){if(!$.isComposing)switch($.key){case"ArrowDown":$.preventDefault(),this.navigateHistory(1);break;case"ArrowUp":$.preventDefault(),this.navigateHistory(-1);break;case"Enter":$.preventDefault(),this.executeCommand();break;case"Escape":$.preventDefault(),this.command="",this.output="",this.showHelp=!1,this.$refs.cmdInput?.blur();break;case"Tab":$.preventDefault(),this.autoComplete();break}},init(){this.currentPath=be(),document.addEventListener("pjax:complete",()=>{this.currentPath=be()}),window.addEventListener("popstate",()=>{this.currentPath=be()}),this.$nextTick(()=>{this.$refs.cmdInput?.focus()})},isPostPage(){return window.haloData?.pageType==="post"},navigateHistory($){if(this.history.length!==0){if(this.historyIndex+=$,this.historyIndex<0&&(this.historyIndex=0),this.historyIndex>=this.history.length){this.historyIndex=this.history.length,this.command="";return}this.command=this.history[this.historyIndex]}}}))}function Sa(){$_.data("typewriter",(_="",$=50)=>({_timerId:null,destroy(){this._timerId!==null&&(window.clearInterval(this._timerId),this._timerId=null)},display:"",init(){let e=0;this.display="";const t=typeof $=="number"?$:50;this._timerId=window.setInterval(()=>{if(e<this.text.length){this.display+=this.text.charAt(e),e+=1;return}this._timerId!==null&&(window.clearInterval(this._timerId),this._timerId=null)},t)},text:_}))}ta(),Sa(),ha(),na(),ra(),JT();const Na=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 8 7 54 0 12 0 64 185
...
`},Symbol.toStringTag,{value:"Module"})),Ca=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 8 6 59 15 10 0 24463
...
\\_|    @
       @@
`},Symbol.toStringTag,{value:"Module"})),Oa=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 8 6 27 0 10 0 576
...
255  LATIN SMALL LETTER Y WITH DIAERESIS
 ...
 _/    _/ $    @
  _/_/_/ $     @
     _/ $      @
_/_/   $       @@
`},Symbol.toStringTag,{value:"Module"})),Ha=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 4 3 10 0 10 0 1920
...
255  LATIN SMALL LETTER Y WITH DIAERESIS
 ...
 \\/$@
 /$ @@
`},Symbol.toStringTag,{value:"Module"})),Ma=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 7 5 16 0 10 0 3904 96
...
255  LATIN SMALL LETTER Y WITH DIAERESIS
 ...
 $\\_/|/@
    /| @
    \\| @@
`},Symbol.toStringTag,{value:"Module"})),Pa=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 5 4 16 0 14 0 4992
...
255  LATIN SMALL LETTER Y WITH DIAERESIS
 ...
 \\__, |$@
 ____/$ @@
`},Symbol.toStringTag,{value:"Module"})),xa=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 6 5 16 15 10 0 18319
...
255  LATIN SMALL LETTER Y WITH DIAERESIS
 ...
 \\__, /   @
/____/    @@
`},Symbol.toStringTag,{value:"Module"})),ya=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 5 4 13 15 10 0 22415
...
255  LATIN SMALL LETTER Y WITH DIAERESIS
 ...
 | || |@
  \\_, |@
  |__/ @@
`},Symbol.toStringTag,{value:"Module"})),Wa=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 6 5 16 15 16
...
_\\__, /   @
/____/    @@
`},Symbol.toStringTag,{value:"Module"})),Da=Object.freeze(Object.defineProperty({__proto__:null,default:`flf2a$ 6 5 16 15 13 0 24463 229
